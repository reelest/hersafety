...
class LocalCache {
    ...
    update(snapshot, policy = LocalCache.ASSUME_DELETED_POLICY) {
        ...
        this.data.splice(
            this.start,
            policy === LocalCache.ASSUME_DELETED_POLICY
                ? this.end - this.start
                : 0,
            ...data
        );
...
async function runSanityChecks(query) {
    ...
    console.assert(
        query.cache.start === (await query.getIndex()),
        "page index"
    );
...
export class MultiQuery {
    ...
    filter(key, op, val) {
        ...
        this.cache.reset().then((e) => this.restart());
        return this;
    }
...
const sendQuery = (dedupeIndex, getState, setState, opts, createQuery) => {
    ...
    const onSuccess = (e) => {
        ...
        if (p === dedupeIndex.current)
            setState({ loading: false, query, data: e, error: null, ...getState() });
    };
    const onError = (e) => {
        console.log({ e, success: false, query });
        if (p === dedupeIndex.current)
                setState({ loading: false, query, error: e, ...getState() });
    };
    if (opts.count)
        query.getCount().then((x) => {
            if (p === dedupeIndex.current)
                setState({ count: x, ...getState() });
        });
    if (opts.index)
        query.getIndex().then((x) => {
            if (p === dedupeIndex.current)
                setState({ index: x, ...getState() });
        });
    if (opts.watch) {
